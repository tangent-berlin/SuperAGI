...
# CRUD Operations
@router.post(
    "/add", response_model=sqlalchemy_to_pydantic(Organisation), status_code=201
)
def create_organisation(
    organisation: sqlalchemy_to_pydantic(Organisation, exclude=["id"]),
...
@router.get(
    "/get/{organisation_id}", response_model=sqlalchemy_to_pydantic(Organisation)
)
...
@router.put(
    "/update/{organisation_id}", response_model=sqlalchemy_to_pydantic(Organisation)
)
...
@router.get(
    "/get/user/{user_id}",
    response_model=sqlalchemy_to_pydantic(Organisation),
    status_code=201,
)
