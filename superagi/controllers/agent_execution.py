...
# CRUD Operations
@router.post(
    "/add", response_model=sqlalchemy_to_pydantic(AgentExecution), status_code=201
)
...
@router.get(
    "/get/{agent_execution_id}", response_model=sqlalchemy_to_pydantic(AgentExecution)
)
def get_agent_execution(
    agent_execution_id: int, Authorize: AuthJWT = Depends(check_auth)
):
...
@router.put(
    "/update/{agent_execution_id}",
    response_model=sqlalchemy_to_pydantic(AgentExecution),
)
def update_agent_execution(
    ...
    if (
        agent_execution.status != "CREATED"
        and agent_execution.status != "RUNNING"
        and agent_execution.status != "PAUSED"
        and agent_execution.status != "COMPLETED"
        and agent_execution.status != "TERMINATED"
    ):
        raise HTTPException(status_code=400, detail="Invalid Request")
...
def agent_list_by_status(status: str, Authorize: AuthJWT = Depends(check_auth)):
    """Get list of all agent_ids for a given status"""

    running_agent_ids = (
        db.session.query(AgentExecution.agent_id)
        .filter(AgentExecution.status == status.upper())
        .distinct()
        .all()
    )
...
def list_running_agents(agent_id: str, Authorize: AuthJWT = Depends(check_auth)):
...
def get_agent_by_latest_execution(
    project_id: int, Authorize: AuthJWT = Depends(check_auth)
):
    ...
    if latest_execution.status == "RUNNING":
        isRunning = True
    agent = (
        db.session.query(Agent).filter(Agent.id == latest_execution.agent_id).first()
    )
    return {
